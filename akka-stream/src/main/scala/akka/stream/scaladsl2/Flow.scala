...
trait Flow[-In, +Out] extends FlowOps[Out] {
   ...
   * the materialized values of the `Source` and `Sink`, e.g. the `Subscriber` of a [[SubscriberTap]] and
   ...
   */
  def runWith(source: SimpleSource[In], sink: KeyedSink[Out])(implicit materializer: FlowMaterializer): sink.MaterializedType =
    source.connect(this).runWith(sink)

  /**
   * Connect the `Source` to this `Flow` and then connect it to the `Sink` and run it.
   *
   * The returned value will contain the materialized value of the `SourceWithKey`, e.g. `Subscriber` of a [[SubscriberTap]].
   */
  def runWith(source: KeyedSource[In], sink: SimpleSink[Out])(implicit materializer: FlowMaterializer): source.MaterializedType =
   ...
   */
  def runWith(source: SimpleSource[In], sink: SimpleSink[Out])(implicit materializer: FlowMaterializer): Unit =
