...
object Sink {
   ...
   */
  def fold[U, In](zero: U, f: japi.Function2[U, In, U]) = KeyedSink(scaladsl2.Sink.fold[U, In](zero)(f.apply))
   ...
   * the stream..
   */
  def foreach[In](f: japi.Procedure[In]) = KeyedSink(scaladsl2.Sink.foreach[In](x ⇒ f(x)))

  /**
   * Helper to create [[Sink]] from `Subscriber`.
   */
  def subscriber[In](subs: Subscriber[In]) = KeyedSink(scaladsl2.Sink(subs))

  /**
   * A `Sink` that materializes into a [[org.reactivestreams.Publisher]].
   * that can handle one [[org.reactivestreams.Subscriber]].
   */
  def publisher[In, M]() = KeyedSink[In, M](scaladsl2.Sink.publisher)
   ...
   */
  def onComplete[In](onComplete: akka.dispatch.OnComplete[Unit]) =
    SimpleSink(scaladsl2.Sink.onComplete[In](x ⇒ onComplete.apply(x)))

  /**
   * A `Sink` that materializes into a `Future` of the first value received.
   */
  def future[In] = KeyedSink[In, Future[In]](scaladsl2.Sink.future[In])
}
/**
